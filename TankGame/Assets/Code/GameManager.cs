...
namespace TankGame
{
	public class GameManager : MonoBehaviour
	{
        ...
        internal void AddScore(int score)
        {
            ...
            _playerlivesUI.UpdateScore(_score, _winningScore);
        }

        public int lives = 3;
        public PlayerStats _playerlivesUI;
        ...
        internal bool Recycle(Unit deadUnit)
        {
            if(deadUnit != _playerUnit)
            {
                // Enemy units are always recycled back to their initial state.
                deadUnit.Recycle();

                // Addind some score from killing an enemy because that makes more sense to me.
                AddScore(5);
            ...
            {
                lives--;
                _playerlivesUI.UpdateLives(lives);

                if (lives == 0)
                {
                    // Zero lives, final death.
                    return false;
                }
                else
                {
